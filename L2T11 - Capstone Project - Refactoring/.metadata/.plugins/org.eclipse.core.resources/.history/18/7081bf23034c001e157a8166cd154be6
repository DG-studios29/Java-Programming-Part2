...
public class QuickFoodDelivery {
 ...
 // Read driver information from a file and return a list of drivers
    ...
    // Find the nearest driver with the smallest load in the same location as the restaurant
    private static Driver findNearestDriver(List<Driver> drivers, String restaurantLocation) {
        Driver nearestDriver = null;
        int minDistance = Integer.MAX_VALUE;

        for (Driver driver : drivers) {
            if (driver.getLocation().equalsIgnoreCase(restaurantLocation) && driver.getLoad() < minDistance) {
                nearestDriver = driver;
                minDistance = driver.getLoad();
            }
        }
    ...
    private static void generateInvoice(Customer customer, Restaurant restaurant, Driver driver) {
        try {
            ...
            if (driver != null) {
                ...
                writer.write("\n" + driver.getName() + " is nearest to the restaurant and will be delivering your order to:\n");
